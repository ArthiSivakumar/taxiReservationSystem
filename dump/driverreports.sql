...
BEGIN
	select cug_no,distance into cugno,dist from booking_details where travel_status='b' and end_of_journey<=CURRENT_TIMESTAMP() order by distance desc limit 0,1;
END$$
DELIMITER ;;
...
BEGIN
	select cug_no,distance into cugno,dist from booking_details where travel_status='b' and end_of_journey<=CURRENT_TIMESTAMP() order by distance limit 0,1;
END$$
DELIMITER ;;
