...
CREATE TRIGGER del_details BEFORE DELETE ON taxi
	...
	BEGIN
		DECLARE travelid INT;
			...
			DECLARE endofjourney TIMESTAMP;
		...
		select `travel_id`,`date_of_booking`,`date_of_journey`,`end_of_journey`,`travel_status`,`source`,`destination`,`distance`,`email_id`,`cug_no`,`taxi_no` from `booking_details` where taxi_no = Old.taxi_no;
		...
		get_rec:LOOP
			IF totalrows = 0 THEN
				LEAVE get_rec;
			END IF;
			FETCH rec_cursor into travelid,dateofbooking,dateofjourney,endofjourney,travelstatus,src,dest,dist,emailid,cugno,taxino;
			insert into deleted_details values(travelid,dateofbooking,dateofjourney,endofjourney,travelstatus,src,dest,dist,emailid,cugno,taxino);		
...
CREATE TRIGGER del_driver_details BEFORE DELETE ON driver
	...
	BEGIN
		...
		DECLARE totalrows INT;
		DECLARE rec_cursor CURSOR FOR
        	select `travel_id`,`date_of_booking`,`date_of_journey`,`end_of_journey`,`travel_status`,`source`,`destination`,`distance`,`email_id`,`cug_no`,`taxi_no` from `booking_details` where cug_no = Old.cug_no;
		DECLARE CONTINUE HANDLER 
	        FOR NOT FOUND SET v_finished = 1;
